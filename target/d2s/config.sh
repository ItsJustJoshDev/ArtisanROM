...
# NOTE: verify TARGET_BOARD_API_LEVEL / TARGET_PLATFORM_SDK_VERSION / TARGET_VNDK_VERSION
# against whatever ROM base branch you're actually building against.
TARGET_BOARD_API_LEVEL=31
TARGET_PLATFORM_SDK_VERSION=32
TARGET_VNDK_VERSION=31
TARGET_SINGLE_SYSTEM_IMAGE="essi"
TARGET_OS_FILE_SYSTEM="erofs"

# Partitions
# !!! UNVERIFIED — these came from the "new patches" zip, not measured against
# your actual d2s partition table. Confirm with `parted -l` / twrp getprop on
# device before flashing anything built with these numbers, or a mismatch here
# can corrupt the install rather than just fail to boot.
...
TARGET_SYSTEM_PARTITION_SIZE=6502219776
TARGET_PRODUCT_PARTITION_SIZE=964689920
...
TARGET_AUDIO_SUPPORT_DUAL_SPEAKER=true # Note10+ has stereo speakers — kept from verified d2s config, "new patches" had this wrong (false)
...
TARGET_COMMON_SUPPORT_EMBEDDED_SIM=false # N975F has no eSIM
...
TARGET_RIL_SIM_CONFIG_MULTISIM_TRAYCOUNT="1" # single-SIM BTU variant — "new patches" had "2", wrong for this model
TARGET_RIL_SUPPORT_WATERPROOF_SIM_TRAY_MSG=true # Note10+ tray is waterproof — "new patches" had this false

TARGET_SECURITY_CONFIG_ESE_CHIP_VENDOR="GEMALTO"
TARGET_SECURITY_CONFIG_ESE_COS_NAME="UT5.0"
TARGET_DISABLE_AVB_SIGNING=true # required for custom-signed builds — was missing entirely from the "new patches" config, restored here

# --- New WLAN schema flags (structural additions from the new patches zip) ---
# All left false/conservative since d2s hardware (exynos9825 era) doesn't support
# Wi-Fi 6E/7, dual-AP, or 6GHz mobile hotspot. Double check against your Wi-Fi
# chip driver if any of these need to flip.
TARGET_WLAN_CONFIG_CONNECTION_PERSONALIZATION="none"
